...
for (i in 1:length(datasets)) {
    ...
    # We want to always run once with uniform priors (without passing in upper argument),
    ...
    rscript_args <- c(here("fit/src/fit_stan_cli.R"), "--model", "R", "--dataset", dataset_name)

    if (fit_params$use_slurm) {
        print("Running slurm")
        system2("srun", c(slurm_args, "Rscript", rscript_args), wait=FALSE)
    } else {
