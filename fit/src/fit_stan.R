        ...
        for (i in 1:length(datasets)) {
            ...
            for (q975 in model_q975s) {
                ...
                if (fit_params$use_slurm) {
                    system2("srun", c("--ntasks=1", "--nodes=1", paste("--cpus-per-task=", fit_params$settings$chains, sep=""), paste("--output=", here("fit", "logs", paste(run_name, "_R.log", sep="")), sep=""), "Rscript", rscript_args), wait=FALSE)
                } else {
